    ...
    <div className="p-6 space-y-6">
      ...
      {isVNRatioLow && (
        <div className="flex items-center gap-3 p-4 bg-destructive/10 border border-destructive/30 rounded-xl animate-pulse">
          ...
          <div>
            <p className="font-semibold text-destructive">Cảnh báo: Tỷ lệ phim Việt Nam thấp!</p>
            <p className="text-sm text-muted-foreground">
              Tỷ lệ hiện tại: {vnRatio.toFixed(1)}%. Yêu cầu tối thiểu: 15% theo quy định.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, index) => (
          <Card key={index} className={cn('relative overflow-hidden', stat.alert && 'border-destructive')}>
          ...
          <CardContent>
            <div className="relative w-40 h-40 mx-auto">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  className={isVNRatioLow ? 'text-destructive' : 'text-green-500'}
                  strokeLinecap="round"
                />
              </svg>
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">Quốc tế ({movies.length - vnMovies.length})</span>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">Mã GD</th>
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">Số tiền</th>
                ...
                {mockTransactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-border hover:bg-muted/50">
                    ...
                    <td className="py-3 px-4">
                      <Badge
                        className={cn(
                          tx.status === 'success' && 'bg-green-100 text-green-700 hover:bg-green-100',
