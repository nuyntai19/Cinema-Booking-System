...
import { movies, cinemas, generateSeatMap } from '@/data/mockData';
...
const SeatSelectionPage: React.FC = () => {
  ...
  const { selectedMovie, selectedShowtime, selectedSeats, addSeat, removeSeat, clearBooking } = useBooking();
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {selectedShowtime.time} • {new Date(selectedShowtime.date).toLocaleDateString('vi-VN')}
              </p>
            </div>
          </div>
          <div className={cn(
        ...
        <div className="grid lg:grid-cols-[1fr,300px] gap-6">
          ...
          <div className="bg-card rounded-xl border border-border p-6">
                ...
                <div key={rowIndex} className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1">
                    {row.map((seat) => (
                      ...
                      >
                        {seat.status === 'sold' ? <X className="w-3 h-3" /> : seat.number}
                      </button>
                    ))}
                  </div>
                  <span className="w-6 text-center text-sm font-medium text-muted-foreground">
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-6 pt-6 border-t border-border">
              ...
              <div className="flex items-center gap-2">
                <div className="seat seat-sold w-6 h-6"><X className="w-3 h-3" /></div>
                    ...
                    <div key={seat.id} className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <span className={cn(
                          ...
                          seat.type === 'couple' ? 'bg-pink-100 text-pink-700' :
